...
export default function ParentPortal() {
  ...
  const { data: attendance = [] } = useQuery({
    ...
    queryFn:  () =>
      child
        ? parentApi
            .getAttendance(child.id, fromDate)
            .then((r) => r.data)
        : [],
    enabled: !!child,
  });
  ...
  const avgMarks =
    marks.length > 0
      ? Math.round(
          marks.reduce(
            (s: number, m: { percentage: number }) => s + m.percentage,
            0
          ) / marks.length
        )
      : 0;
  ...
  const pendingFees = fees.filter(
    (f: { status: string }) =>
      f.status === "due" || f.status === "overdue"
  );
  const totalDue = pendingFees.reduce(
    (s: number, f: { balance_due: number }) => s + f.balance_due,
    0
  );
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        {child && (
          <div className="bg-white border border-slate-200 rounded-xl p-4
            ...
            {/* AI risk indicator from latest agent log */}
          ...
          <TabsList className="bg-slate-100">
            ...
            <TabsTrigger value="alerts"      className="text-sm">
              ...
              {unreadNotifs.length > 0 && (
                <Badge className="ml-1.5 h-4 w-4 p-0 text-xs justify-center
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              ...
              {marks.length === 0 ? (
                ...
                <div className="divide-y divide-slate-50">
                  {marks.map((m: {
                    id: number; subject: string; exam_type: string;
                    exam_date: string; percentage: number;
          ...
          <TabsContent value="fees" className="mt-4">
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              ...
              {fees.length === 0 ? (
                ...
                <div className="divide-y divide-slate-50">
                  {fees.map((f: {
                    id: number; fee_type: string; amount: number;
                    balance_due: number; due_date: string;
                      ...
                      <div key={f.id}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">
                              {isPaid
                                ? `Paid on ${formatDate(f.paid_date!)}`
          ...
          <TabsContent value="alerts" className="mt-4">
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              ...
              {notifications.length === 0 ? (
                ...
                <div className="divide-y divide-slate-50">
                  {notifications.map((n: {
                    id: number; notification_type: string;
                    content: string; channel: string; created_at: string;
                    triggered_by?: string;
                  }) => (
